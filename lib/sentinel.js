...
var Sentinel = function(port, host, callback) {
  if(false === this instanceof Sentinel) {
    return new Sentinel(host, port, callback);
  }
  ...
  this.commandClient = redis.createClient(host, port);

  this.commandClient.on('error', function(err) {
    console.log('Redis Sentinel Error: ' + err);
  });

  this.subscribingClient = redis.createClient(host, port);
...
Sentinel.prototype.handler = function() {
  ...
  return function(p, ch, msg) {
    ...
    function _parseInstanceDetails(details) {
      ...
      //     ret['master-name'] = master[1];
      //     ret['master-ip'] = master[2];
      //     ret['master-port'] = master[3];
    ...
    switch(ch) {
      ...
      case '-slave-reconf-undo':
        self.emit('slave-reconf-state', {state: 'aborted', details: _parseInstanceDetails(msg)});
      break;
      ...
      case '+sentinel':
        self.emot('new-sentinel', {details: _parseInstanceDetails(msg)});
      break;
      ...
      case '+switch-master':
        self.emit('switch-master', {status: 'switch-master', details: _parseSwitchMasterDetails(msg)});
      break;
...
Sentinel.prototype.isLeader = function(masterHost, masterPort, callback) {
  ...
  self.isMasterDown(masterHost, masterPort, function(err, infoMaster) {
    ...
    self.commandClient.info(function(err, info) {
      if(err) return callback(err);
      callback(null, info.indexOf('run_id:' + infoMaster.leaderSentinel) !== -1);
    });
  })
};
